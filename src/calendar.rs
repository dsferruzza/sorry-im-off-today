...
fn parse_event(event: &IcalEvent) -> Option<Event> {
    ...
    let confirmed = props.clone().any(|p| p.name == "STATUS" && p.value == Some(String::from("CONFIRMED")));
    if confirmed {
        let name = props.clone().find(|p| p.name == "SUMMARY").and_then(|p| p.clone().value);
        let start = props.clone().find(|p| p.name == "DTSTART").and_then(|p| p.clone().value).as_ref().map(|v| v.as_str()).and_then(parse_date);
        let end = props.clone().find(|p| p.name == "DTEND").and_then(|p| p.clone().value).as_ref().map(|v| v.as_str()).and_then(parse_date);
...
fn parse_date(date: &str) -> Option<DateTime<Utc>> {
    ...
    let parsed_date = NaiveDateTime::parse_from_str(date, &format)
        ...
        .or_else(|_e| NaiveDate::parse_from_str(date, &format4).map(|nd| nd.and_time(NaiveTime::from_hms(0, 0, 0))))
